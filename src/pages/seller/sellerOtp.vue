            ...
            <div class="seller-signup-form">
                <div class="signup-form-inner-content">
                    <div class="heading">
                        ...
                        <br>
                        <h4>SELLER ACCOUNT VERIFICATION</h4>
                    </div>
                    <div class="form-part">
                        <form @submit.prevent="myform" action="">
                            <InputField 
                            :value.sync="seller.otp"
                            type="text"
                            label="OTP"
                            ref="otp"
                            @checkfail="checker"
...
export default {
    ...
    mounted() {
        if(this.$store.state.temporarySeller==null)
            this.$router.push({name:'sellerSignup'})
    },
    methods: {
        verifyOtp(){
            
            let phone=this.$store.state.temporarySeller.phone;
            let email=this.$store.state.temporarySeller.email;
            ...
            this.$http.post('seller/otp/verify',formData)
             .then(response=>{
